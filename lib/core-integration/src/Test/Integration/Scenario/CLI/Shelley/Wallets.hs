...
spec = describe "SHELLEY_CLI_WALLETS" $ do
    it "BYRON_GET_03 - Shelley CLI does not show Byron wallet" $ \ctx -> runTest $ do
    ...
    it "BYRON_LIST_03 - Shelley CLI does not list Byron wallet" $ \ctx -> runTest $ do
    ...
    it "BYRON_DELETE_03 - Shelley CLI does not delete Byron wallet" $ \ctx -> runTest $ do
        wid <- emptyRandomWallet' ctx
    ...
    it "BYRON_WALLETS_UTXO -\
        \ Cannot show Byron wal utxo with shelley CLI" $ \ctx -> runTest $ do
        wid <- emptyRandomWallet' ctx
    ...
    it "BYRON_WALLETS_UPDATE_PASS -\
        \ Cannot update Byron wal with shelley CLI" $ \ctx -> runTest $ do
        wid <- emptyRandomWallet' ctx
    ...
    it "BYRON_WALLETS_UPDATE -\
        \ Cannot update name Byron wal with shelley CLI" $ \ctx -> runTest $ do
        wid <- emptyRandomWallet' ctx
        let port = T.pack $ show $ ctx ^. typed @(Port "wallet")
        let args = T.unpack <$>
                [ "wallet", "update", "name"
    ...
    it "WALLETS_CREATE_01,08 - Can create a wallet" $ \ctx -> runTest $ do
    ...
    it "WALLETS_CREATE_02 - Restored wallet preserves funds" $ \ctx -> runTest $ do
    ...
    it "WALLETS_CREATE_03 - Cannot create wallet that exists" $ \ctx -> runTest $ do
    ...
    describe "WALLETS_CREATE_04 - Wallet names" $ do
        forM_ walletNames $ \(title, n) -> it title $ \ctx -> runTest $ do
    ...
    describe "WALLETS_CREATE_04 - Wallet names invalid" $ do
        forM_ walletNamesInvalid $ \(name, expects) -> it expects $ \ctx -> runTest $ do
            Stdout m <- generateMnemonicsViaCLI @t ["--size", "18"]
    ...
    describe "WALLETS_CREATE_05 - Can create wallet with different mnemonic sizes" $ do
        forM_ ["15", "18", "21", "24"] $ \(size) -> it size $ \ctx -> runTest $ do
    ...
    describe "WALLETS_CREATE_05 - Can't create wallet with wrong size of mnemonic" $ do
        forM_ ["9", "12"] $ \(size) -> it size $ \ctx -> runTest $ do
            let name = "Wallet created via CLI"
    ...
    describe "WALLETS_CREATE_07 - Passphrase is valid" $ do
        ...
        forM_ matrix $ \(title, pass) -> it title $ \ctx -> runTest $ do
            Stdout m <- generateMnemonicsViaCLI @t []
    ...
    describe "WALLETS_CREATE_07 - When passphrase is invalid" $ do
        ...
        forM_ matrix $ \(title, pass, warn) -> it title $ \ctx -> runTest $ do
            Stdout m <- generateMnemonicsViaCLI @t []
            (c, o, e) <- createWalletViaCLI @t ctx ["Wallet name"] m "\n" pass
            c `shouldBe` ExitFailure 1
            o `shouldBe` ""
    ...
    describe "WALLETS_CREATE_08 - --address-pool-gap values" $ do
        ...
        forM_ matrix $ \(title, gap, expects) -> it title $ \ctx -> runTest $ do
            Stdout m <- generateMnemonicsViaCLI @t []
            (c, o, e) <- createWalletViaCLI @t ctx ["n", "--address-pool-gap", gap]
                            m "\n" "secure-passphraze"
            expects c o e gap

    it "WALLETS_GET_01 - Can get a wallet" $ \ctx -> runTest $ do
    ...
    describe "WALLETS_GET_03,04 - Cannot get wallets with false ids" $ do
        forM_ falseWalletIds $ \(title, walId) -> it title $ \ctx -> runTest $ do
            (Exit c, Stdout out, Stderr err) <- getWalletViaCLI @t ctx walId
            out `shouldBe` ""
            c `shouldBe` ExitFailure 1
    ...
    it "WALLETS_LIST_01 - Can list wallets" $ \ctx -> runTest $ do
    ...
    it "WALLETS_LIST_01 - Wallets are listed from oldest to newest" $ \ctx -> runTest $ do
    ...
    describe "WALLETS_UPDATE_PASS_02 - New passphrase values" $ do
        ...
        forM_ matrix $ \(title, ppNew, expectations) -> it title $ \ctx -> runTest $ do
            let name = "name"
            let ppOld = "old secure passphrase"
            let addrPoolMin = fromIntegral @_ @Int $ getAddressPoolGap minBound
    ...
    describe "WALLETS_UPDATE_PASS_03 - Old passphrase values" $ do
        ...
        forM_ matrix $ \(title, ppOldWrong, expectations) -> it title $ \ctx -> runTest $ do
            let name = "name"
    ...
    describe "WALLETS_UPDATE_PASS_03 - \
        ...
        forM_ matrix $ \(title, ppOldRight, expectations) -> it title $ \ctx -> runTest $ do
            let name = "name"
    ...
    describe "WALLETS_UPDATE_PASS_04 - Cannot update pass of wallets with false ids" $ do
        forM_ falseWalletIds $ \(title, wid) -> it title $ \ctx -> runTest $ do
    ...
    describe "WALLETS_UPDATE_PASS_05,06 - \
        ...
        forM_ matrix $ \(title, pass, expectations) -> it title $ \ctx -> runTest $ do
            wSrc <- fixtureWallet ctx
    ...
    it "WALLETS_DELETE_01, WALLETS_LIST_02 - Can delete wallet" $ \ctx -> runTest $ do
        walId <- emptyWallet' ctx
    ...
    it "WALLETS_UTXO_01 - Wallet's inactivity is reflected in utxo" $ \ctx -> runTest $ do
        wid <- emptyWallet' ctx
        (Exit c, Stdout o, Stderr e) <- getWalletUtxoStatisticsViaCLI @t ctx wid
        c `shouldBe` ExitSuccess
        e `shouldBe` cmdOk
    ...
    it "WALLETS_UTXO_02 - Utxo statistics works properly" $ \ctx -> runTest $ do
        wSrc <- fixtureWallet ctx
        wDest <- emptyWallet ctx
    ...
    it "WALLETS_UTXO_03 - Deleted wallet is not available for utxo" $ \ctx -> runTest $ do
        wid <- emptyWallet' ctx
    ...
    it "WALLETS_UTXO_03 - 'almost' valid walletId" $ \ctx -> runTest $ do
        wid <- emptyWallet' ctx
        (Exit c, Stdout o, Stderr e)
...
emptyRandomWallet'
    :: (MonadIO m, MonadCatch m, MonadMask m, UnliftIO m)
    => Context t
    -> ResourceT m String
